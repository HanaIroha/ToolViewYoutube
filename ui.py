...
class UI:
    def __init__(self):
        ...
        self.tiles = "data/systems/Artificial.pcx"
        self.logo  = "data/systems/intelligence.pcx"
        self.window_width = 1200
        self.window_height = 700
        self.log = _log.Logging('HANGANSTUDIO')
        with open("version","r",encoding="utf8") as fo:
            self.version = fo.read().strip()

        self.log.logging('info','Chào mừng bạn đến với trình quản lý YouTube smart phone')

        x_start = 20
        y_start = self.window_height / 3 + 40
        y_delta = 30
        ...
        points = [
                #cau hinh chung
                [10,ws[1]/3-20,  500,ws[1]/3-20],
                [10,ws[1]/3-20,  10,400],
                [500,ws[1]/3-20, 500,400],
                [10,ws[1]/3+167, 500,400],
                
                #lua chon tinh nang
                [10,ws[1]/2+130, 380,450+30],
                [10,ws[1]/2+130, 10,ws[1]-40],
                [10,ws[1]-40,    380,ws[1]-40],
                [380,450+30,     380,ws[1]-40],
...
#
        ...
        menu_bar.add_cascade(label="Thiết lập ♚", menu=settings_menu)
        ...
        menu_bar.add_cascade(label="Giới Thiệu ♛", menu=about_menu)
        about_menu.add_command(label="☞ Phiên bản: "+self.version)
        about_menu.add_command(label="☞ Cập Nhật")
        about_menu.add_command(label="☎ Hà Ngân Studio")
        ...
        Label(r,text="Cấu hình chung").place(x=10,y=ws[1]/3-42)
        Label(r,text="Lựa chọn tính năng").place(x=10,y=ws[1]/3*2-10)
        Label(r,text="Danh sách videos").place(x=ws[0]/2-75,y=15)
        Label(r,text="Danh sách comment").place(x=ws[0]/2-75,y=179)
        Label(r,text="Từ khóa tìm kiếm").place(x=ws[0]/2-75,y=338)
        Label(r,text="Danh sách proxy").place(x=ws[0]/2-75,y=500)
        ...
        def _start():
            ...
            isChangeEmail = var_changeemail_bool.get()
            ...
            if ranb and (likb or subb or cmtb or dlib or usub or seab):
                mb.showerror("Lỗi Hệ Thống", "Option được chọn không hợp lệ !")
                return
            ...
            if not os.path.exists("data/configure.json"):
                (self.log).logging('fatal','File thiết lập không tồn tại')
                os._exit(-2)
            else:
                ...
                _jout['bool:changemail'] = isChangeEmail
                with open("data/configure.json","w+") as fi:
                    fi.write(json.dumps(_jout,indent=2))

            changeipbyproxy = True if chab else False
            
            (self.log).logging('info','Hệ thống đang được thiết lập ... vui lòng chờ')
            ...
            while i < number_threads:
                try:
                    thread_id = i
                    threads[i] = threading.Thread(target=youtab.YouTab,args=(thread_id,ip_splited[thread_id],changeipbyproxy))
                    threads[i].start()
                except Exception as err:
                    (self.log).logging('fatal','Xem video %s đã lỗi'%i)
                (self.log).sleep(_tsleep,note='trước khi bắt đầu video tiếp theo')
                i += 1
        ...
        var_changeemail_bool = IntVar()

        var_random_bool.set(1)

        Checkbutton(r,text="Random",variable=var_random_bool).place(x=40,y=ws[1]/3*2+40)
        Checkbutton(r,text="Comment",variable=var_comment_bool).place(x=150,y=ws[1]/3*2+40)
        Checkbutton(r,text="Like",variable=var_like_bool).place(x=40,y=ws[1]/3*2+40+50*1)
        Checkbutton(r,text="Dislike",variable=var_dislike_bool).place(x=150,y=ws[1]/3*2+40+50*1)
        Checkbutton(r,text="Subscribe",variable=var_subscribe_bool).place(x=40,y=ws[1]/3*2+40+50*2)
        Checkbutton(r,text="Unsubscribe",variable=var_unsubscribe_bool).place(x=150,y=ws[1]/3*2+40+50*2)
        Checkbutton(r,text="Search",variable=var_search_bool).place(x=270,y=ws[1]/3*2+40*1)
        Checkbutton(r,text="Change IP",variable=var_changeip_bool).place(x=270,y=ws[1]/3*2+40+50*1)        
        Checkbutton(r,text="Change Email",variable=var_changeemail_bool).place(x=270,y=ws[1]/3*2+40+50*2)   
    ...
    def check_update(self):
        ...
        textbox = Text(frame_devices,width=32,height=38)
        textbox.pack()
        ...
        scrollbar.pack(side=RIGHT,fill=Y)
        textbox = Text(frame_videos,width=44,height=8)
        textbox.pack()
        ...
        frame_comments.place(x=self.ws[0]/2-75,y=200)
        scrollbar = Scrollbar(frame_comments)
        scrollbar.pack(side=RIGHT,fill=Y)
        textbox = Text(frame_comments,width=44,height=8)
        textbox.pack()
        ...
        frame_ips.place(x=self.ws[0]/2-75,y=523)
        scrollbar = Scrollbar(frame_ips)
        scrollbar.pack(side=RIGHT,fill=Y)
        textbox=Text(frame_ips,width=44,height=8)
        textbox.pack()
        ...
        frame_keywords.place(x=self.ws[0]/2-75,y=360)
        scrollbar = Scrollbar(frame_keywords)
        scrollbar.pack(side=RIGHT,fill=Y)
        textbox=Text(frame_keywords,width=44,height=8)
        textbox.pack()
        textbox.config(yscrollcommand=scrollbar.set)
        scrollbar.config(command=textbox.yview)
    ...
    def configure_delay(self):
        ...
        child_window.title("Thiết lập thời gian hành động diễn ra")
        ws = (400,250)
        ...
        def set_value():
            ...
            output_json['delay:sec:like']        = delay_before_like
            output_json['delay:sec:dislike']     = delay_before_dislike
            output_json['delay:sec:subscribe']   = delay_before_subscribe
            output_json['delay:sec:unsubscribe'] = delay_before_unsubscribe
            output_json['delay:sec:comment']     = delay_before_comment
    ...
    def configure_probability(self):
        child_window=Toplevel()
        child_window.title("Thiết lập xác suất khi xem video")
    ...
    def restart(self):
        dl = adb.device_list()
        for di in dl:
            (self.log).logging('info','Khởi động lại điện thoại -> %s'%di.serial)
